...
def addToList(self):
    ...
    # normalize the data to keep it consistent in database
    var_fname = var_fname.strip() # normalize by removing blanks
    var_lname = var_lname.strip() # normalize by removing blanks
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # enforse the user to provide both first and last name, phone and email.
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) #call the function to clear all the textboxes and the select index of listbox
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        messagebox.showerror("Missing information","Please select a neame from the list. \nThen edit the phone or email information.")
    onClear(self)
